...
static int lowrisc_mii_probe(struct net_device *ndev)
{
  ...
  // enum {phyaddr=1};
	...
	phydev = mdiobus_get_phy(priv->mii_bus, phyaddr);
...
//  printk("ndev.name %s\n", ndev->name);
//  printk("============\n");
//  printk("priv->mii_bus[phyaddr] %s\n", priv->mii_bus[phyaddr]);
//  printk("============\n");
//  printk("priv->mii_bus[phyaddr]->addr %s\n", priv->mii_bus[phyaddr].addr);
//  printk("============\n");
...
//====================================
	...
	if (!phydev) {
          netdev_err(ndev, "no PHY found at address %d\n", phyaddr);
          // netdev_err(ndev, "no PHY found at address %d\n", 1);
		return -ENODEV;
	}
...
static int lowrisc_open(struct net_device *ndev)
{
  ...
  //=========  debugging  ===========
  printk("priv->ndev->name %s\n", (priv->ndev)->name);
  printk("ndev->name %s\n", (ndev)->name);
  printk("ndev->dev_addr %s\n", ndev->dev_addr);
